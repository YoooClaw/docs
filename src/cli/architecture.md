...
入站永远是同一组 ingest 端点（`POST /notifications` `/images`、`/gateway/recordings.result.write`），谁来「喂」由模式决定。出站抽象成 Egress 端口：`standalone` 走 Relay 隧道（`RelayEgress`）、`proxied` POST 到 `--egress-callback-url`（`ProxyEgress`）、`direct` 丢弃（`NoopEgress`），替换了原先散落的隧道 PushEvent 调用。
...
| `/notifications` `/images` | POST | gateway token / api-key / Relay 内部头 | 手机端数据写入 |
| `/gateway/<method>` | POST | gateway token / api-key / Relay 内部头 | gateway 方法桥（含录音 `recordings.result.write`） |
...
recordings.result.write（转写/总结，可选 ossUrl）
  ↓
recordings/index.json 落元数据（新建则 status=synced）
  ↓
写 transcript-data/*.json + transcripts/*.md + summaries/*.md
  ↓
可选：带 ossUrl 时后台下载音频到 recordings/audio/
  ↓  status=transcribed
state/events.jsonl 追加状态事件，并可经 Relay 推回手机端
（本机重新转写经 recordings.retranscribe 触发 ASR api/model-proxy）
```
