...
- `label`：隧道与落盘标识，必须匹配 `[a-z0-9-]{1,32}`，且不能用保留字 `all` / `legacy` / `env` / `keychain` / `local`（见 [internal/creds/store.go](https://github.com/YoooClaw/cli/blob/master/internal/creds/store.go) 的校验逻辑）。
...
daemon 启动后由 [internal/relay/supervisor.go](https://github.com/YoooClaw/cli/blob/master/internal/relay/supervisor.go) 按 `CredentialSet` 编排隧道——**每个 label 一条 `RelayClient`**：
...
入站请求在鉴权阶段就确定了来源 `clientLabel`（见 [internal/daemon/server.go](https://github.com/YoooClaw/cli/blob/master/internal/daemon/server.go)）：
...
- [架构与实现逻辑](/cli/architecture) —— daemon、Relay、Dispatcher 全貌。
